...
const ChartTooltip = ({
  ...
  if (active && payload && payload.length) {
    // Filter out payload items with 0% usage
    const filteredPayload = payload.filter((item) => item.value > 0);

    // If no payload items remain after filtering, do not display the tooltip
    ...
    return (
      <div
        ...
        <div className={cx("space-y-1 px-4 py-2")}>
          {filteredPayload.map(({ value, category, color }, index) => (
            <div
              key={`id-${index}`}
